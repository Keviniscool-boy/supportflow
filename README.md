...
> **项目状态：** 已完成 PRD、技术架构和开发任务拆分；数据库与 API 设计文档仍处于计划稿阶段。下一阶段进入编码，v0.2 的目标是可自托管 Demo，不是企业生产版本。
...
当前尚未发布可运行应用。项目严格按照 PRD → 技术架构 → 数据库 → API → Task → 编码 → 测试 → 部署的顺序推进。
...
Docker Compose 和一键 Demo 初始化将在 v0.2 实现阶段提供。
